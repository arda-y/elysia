"""match_voice_files_v8.py - v7 + resolving genuinely new-JV-only
branches directly against JV, bypassing the TFC pipeline entirely.

New in v8: 269 files belong to branches that don't exist in the old
(TFC) db at all - confirmed genuinely new-JV-only content (Helen's
Easter Eggs, Gurdis and the Mountain Goats, Modus Mullen, etc), not a
parsing bug. Since these never resolve an old (branch_id, dentry_id) in
the first place, there's zero collision risk in matching them straight
against JV instead - nothing in the TFC-resolve pipeline ever touches
them either way. Two-step:
  1. Resolve the branch title directly against JV - exact match first,
     then a word-subsequence fuzzy fallback (the file's branch words
     must appear, in order, within the JV title's words - handles the
     "hidden extra word" pattern found twice by hand, e.g. a JV title
     with an unlisted "Flow – " prefix the filename doesn't carry).
  2. Same-number/same-actor check, straight against JV this time (no
     TFC old_n to translate) - does a JV dentry exist at the file's own
     N, in the resolved branch, spoken by the real (folder) actor?
Checked at scale before trusting it: 267/268 branch-resolved files
(99.6%!) hit this way - far higher than the same check's ~18-42% hit
rate elsewhere, because this content never existed in TFC to begin
with, so there's no TFC-era renumbering for its N to have drifted from -
it was authored once, directly against JV's own numbering.
...
Usage: python3 match_voice_files_v8.py <old.db> <jv.db> <sound.db>
"""
import re
import sqlite3
import sys
...
def stage4_jv_direct(sound_rows, sound_meta, old_title_to_cids, jv_title_to_cids, jv_dentries, already_resolved):
    """For files whose branch doesn't exist in the old db at all (so
    stage1 never even produces an (old_cid, old_n) for them) - resolve
    the branch directly against JV (exact, then word-subsequence fuzzy),
    then try the same-number/same-actor check straight against JV."""
    jv_titles_words = {t: t.split() for t in jv_title_to_cids}
    extra = {}
    stats = defaultdict(int)
    ...
    for sound_id, rel_path, filename in sound_rows:
        ...
        branch_text, n, alt_index = parsed
        branch_key = normalize_title(branch_text)
        if branch_key in old_title_to_cids:
            continue  # has old-side data, not this tier's job

        jv_cids = jv_title_to_cids.get(branch_key)
        if not jv_cids:
            needle = branch_key.split()
            matches = [t for t, words in jv_titles_words.items() if is_subsequence(needle, words)]
            if len(matches) == 1:
                jv_cids = jv_title_to_cids[matches[0]]
            elif len(matches) > 1:
                stats["jv_fuzzy_title_ambiguous"] += 1
                continue
            else:
                stats["jv_title_unresolved"] += 1
                continue
        if len(jv_cids) != 1:
            stats["jv_title_ambiguous"] += 1
            continue
        jv_cid = jv_cids[0]

        real_actor_n = tight_normalize_actor(folder_actor) if folder_actor else None
        if not real_actor_n or real_actor_n == "ALTERNATIVE":
            stats["no_real_actor"] += 1
            continue

        entry = jv_dentries.get((jv_cid, n))
        if not entry or not entry[0]:
            stats["jv_direct_no_dentry_or_actor"] += 1
            continue
        if tight_normalize_actor(entry[0]) != real_actor_n:
            stats["jv_direct_actor_mismatch"] += 1
            continue

        extra[sound_id] = (jv_cid, n)
        stats["resolved_jv_direct"] += 1
...
if __name__ == "__main__":
    ...
    extra4, s4_stats = stage4_jv_direct(sound_rows, sound_meta, old_title_to_cids, jv_title_to_cids, jv_dentries, final)
    print("\n--- stage 4 (new-JV-only branches, resolved directly against JV) ---")
    for k, v in s4_stats.items():
        print(f"  {k:35s}: {v}")
    final.update(extra4)
    ...
    import json
    with open("/tmp/v8_final_result.json", "w") as f:
        json.dump({str(k): v for k, v in final.items()}, f)
    print("wrote /tmp/v8_final_result.json")
